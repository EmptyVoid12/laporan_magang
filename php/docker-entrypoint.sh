...
# Existing .env should be preserved to avoid unexpectedly changing app/database settings.
...
# Step 8: Run project initialization only once
INIT_LOCK_FILE="/var/www/html/storage/app/project_initialized.lock"
if [ ! -f "$INIT_LOCK_FILE" ]; then
  echo "🚀 Running one-time project initialization..."
  if php artisan project:init; then
    touch "$INIT_LOCK_FILE"
    echo "✅ Project initialization complete. Future container restarts will skip this step."
  else
    echo "⚠️ Project initialization failed. Lock file was not created."
  fi
else
  echo "⏭️ Project already initialized. Skipping project:init."
fi
