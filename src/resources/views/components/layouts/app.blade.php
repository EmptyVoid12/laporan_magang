            ...
            <div class="flex h-16 items-center justify-between px-4 lg:px-6">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="hidden items-center gap-2 border-l border-slate-200 pl-3 sm:flex">
                        <div class="flex h-8 w-8 items-center justify-center rounded-full bg-slate-200 text-xs font-bold text-slate-600">{{ strtoupper(substr($actor->name, 0, 1)) }}</div>
                        <div>
                            <div class="text-sm font-semibold text-slate-800">{{ $actor->name }}</div>
                            <div class="text-[11px] capitalize text-slate-400">{{ $actor->role }}</div>
                        </div>
                    </div>
                    <form method="POST" action="{{ Auth::guard('admin')->check() ? route('adminnoc.logout') : route('logout') }}">
                        @csrf
        ...
        <div class="flex min-h-[calc(100vh-4rem)]">
            ...
            <aside class="hidden w-56 shrink-0 border-r border-slate-200 bg-white lg:block">
                <nav class="space-y-0.5 px-2 py-4">
                    @if($actor->role === 'admin')
                        @if(!request()->routeIs('adminnoc.portal'))
                        ...
                        @endif
                        <a href="{{ route('adminnoc.portal') }}" class="sidebar-link flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-bold text-slate-600 {{ request()->routeIs('adminnoc.portal') && !request()->has('tab') ? 'active' : '' }}">
                        ...
                        <a href="{{ route('adminnoc.portal', ['tab' => 'perangkat']) }}" class="sidebar-link flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-600 {{ (request()->routeIs('adminnoc.portal') && request()->query('tab') === 'perangkat') || request()->routeIs('admin.perangkat') ? 'active' : '' }}">
                        ...
                        <a href="{{ route('adminnoc.portal', ['tab' => 'gangguan']) }}" class="sidebar-link flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-600 {{ (request()->routeIs('adminnoc.portal') && request()->query('tab') === 'gangguan') || request()->routeIs('admin.gangguan') ? 'active' : '' }}">
                ...
                <aside class="relative w-64 bg-white shadow-xl h-full overflow-y-auto">
                    ...
                    <nav class="space-y-0.5 px-2 py-3">
                        @if($actor->role === 'admin')
                            ...
                            @endif
                            <a href="{{ route('adminnoc.portal') }}" class="sidebar-link block rounded-lg px-3 py-2.5 text-sm font-bold text-slate-600 {{ request()->routeIs('adminnoc.portal') && !request()->has('tab') ? 'active' : '' }}" onclick="document.getElementById('mobileSidebar').classList.add('hidden')">Portal Admin</a>
                            <a href="{{ route('adminnoc.portal', ['tab' => 'perangkat']) }}" class="sidebar-link block rounded-lg px-3 py-2.5 text-sm font-medium text-slate-600 {{ (request()->routeIs('adminnoc.portal') && request()->query('tab') === 'perangkat') || request()->routeIs('admin.perangkat') ? 'active' : '' }}" onclick="document.getElementById('mobileSidebar').classList.add('hidden')">Perangkat</a>
                            <a href="{{ route('adminnoc.portal', ['tab' => 'gangguan']) }}" class="sidebar-link block rounded-lg px-3 py-2.5 text-sm font-medium text-slate-600 {{ (request()->routeIs('adminnoc.portal') && request()->query('tab') === 'gangguan') || request()->routeIs('admin.gangguan') ? 'active' : '' }}" onclick="document.getElementById('mobileSidebar').classList.add('hidden')">Laporan Masuk</a>
                        @elseif($actor->role === 'operator')
