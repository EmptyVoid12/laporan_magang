...
<?php $component->withAttributes([]); ?>
        <div class="space-y-5">
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
                    <div class="rounded-xl border border-red-100 bg-red-50 px-4 py-3 dark:border-red-500/20 dark:bg-red-500/10">
                        ...
                        <div class="mt-1 text-sm font-bold text-slate-900 dark:text-white">
                            <?php echo e($topHotspot['nama_perangkat'] ?? 'Belum ada riwayat laporan'); ?>

                        </div>
                        <div class="text-xs text-slate-500 dark:text-slate-400">
                            <?php echo e($topHotspot['total_laporan'] ?? 0); ?> laporan
                        </div>
                    </div>
                    ...
                    <div class="rounded-xl border border-sky-100 bg-sky-50 px-4 py-3 dark:border-sky-500/20 dark:bg-sky-500/10">
                        ...
                        <div class="mt-1 text-sm font-bold text-slate-900 dark:text-white"><?php echo e($mappedDeviceCount); ?></div>
                    ...
                    <div class="rounded-xl border border-amber-100 bg-amber-50 px-4 py-3 dark:border-amber-500/20 dark:bg-amber-500/10">
                        <div class="text-[11px] font-semibold uppercase tracking-wide text-amber-700 dark:text-amber-300">
                            <?php echo e($hasHotspotData ? 'Titik Aktif' : 'Hotspot Tersedia'); ?>

                        </div>
                        <div class="mt-1 text-sm font-bold text-slate-900 dark:text-white"><?php echo e($hasHotspotData ? $activeHotspotCount : $hotspotCount); ?></div>
                        <div class="text-xs text-slate-500 dark:text-slate-400">
                            <?php echo e($hasHotspotData ? 'Masih memiliki aduan yang belum selesai' : 'Akan terisi otomatis setelah laporan gangguan masuk'); ?>
            ...
            <!--[if BLOCK]><![endif]--><?php if($mappedDeviceCount === 0): ?>
            ...
            <?php else: ?>
                <div
                    x-data="{
                        hotspots: <?php echo \Illuminate\Support\Js::from($hotspots)->toHtml() ?>,
                        heatmapData: <?php echo \Illuminate\Support\Js::from($heatmapData)->toHtml() ?>,
                        mapCenter: <?php echo \Illuminate\Support\Js::from($mapCenter)->toHtml() ?>,
                        loadError: false,
                        init() {
                    ...
                    x-init="init()"
                >
                    <!--[if BLOCK]><![endif]--><?php if (! ($hasHotspotData)): ?>
                            ...
                            <div class="mb-3 flex items-center justify-between">
                                <div>
                                    <h4 class="text-sm font-bold text-slate-900 dark:text-white">
                                        <?php echo e($hasHotspotData ? 'Hotspot Tertinggi' : 'Perangkat Terpetakan'); ?>

                                    </h4>
                                    <p class="text-xs text-slate-500 dark:text-slate-400">
                                        <?php echo e($hasHotspotData ? 'Urut berdasarkan jumlah laporan gangguan.' : 'Menampilkan perangkat yang sudah memiliki koordinat.'); ?>
                                ...
                                <span class="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-600 dark:bg-white/10 dark:text-slate-300">
                                    <?php echo e($hasHotspotData ? $hotspotCount : $mappedDeviceCount); ?> titik
                                </span>
                            </div>

                            <div class="space-y-3">
                                <!--[if BLOCK]><![endif]--><?php $__currentLoopData = collect($hotspots)->take(5); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $hotspot): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-white/10 dark:bg-white/5">
                                        <div class="flex items-start justify-between gap-3">
                                            <div>
                                                <div class="text-sm font-bold text-slate-900 dark:text-white">
                                                    <?php echo e($hotspot['rank'] ? '#'.$hotspot['rank'].' ' : ''); ?><?php echo e($hotspot['nama_perangkat']); ?>

                                                </div>
                                                <div class="mt-1 text-[11px] uppercase tracking-wide text-slate-500 dark:text-slate-400">
                                                    <?php echo e($hotspot['jenis_perangkat']); ?> • <?php echo e($hotspot['wilayah'] ?: '-'); ?>

                                                </div>
                                                <div class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                                                    <?php echo e($hotspot['lokasi']); ?>

                                                </div>
                                            </div>

                                            <div class="text-right">
                                                <div class="text-lg font-black text-slate-900 dark:text-white"><?php echo e($hotspot['total_laporan']); ?></div>
                                        ...
                                        <div class="mt-3 flex items-center justify-between text-xs">
                                            <span class="rounded-full bg-amber-100 px-2.5 py-1 font-semibold text-amber-700 dark:bg-amber-500/10 dark:text-amber-300">
                                                <?php echo e($hasHotspotData ? 'Aktif: '.$hotspot['aduan_aktif'] : 'Koordinat siap dipakai'); ?>

                                            </span>
                                            <span class="text-slate-400 dark:text-slate-500">
                                                <?php echo e($hotspot['laporan_terakhir'] ?: ($hasHotspotData ? 'Belum ada waktu laporan' : 'Belum ada riwayat laporan')); ?>
...
<?php endif; ?>
<?php /**PATH /var/www/html/resources/views/filament/admin/widgets/map-dashboard-widget.blade.php ENDPATH**/ ?>