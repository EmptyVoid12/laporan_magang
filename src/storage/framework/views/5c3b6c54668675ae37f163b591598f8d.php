            ...
            <div class="flex h-16 items-center justify-between px-4 lg:px-6">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div x-data="{ open: false }" class="relative" @click.away="open = false">
                        ...
                        <button @click="open = !open" class="relative rounded-lg p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-700 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-350">
                            ...
                            <?php if($unreadCount > 0): ?>
                                <span class="absolute -right-0.5 -top-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white"><?php echo e($unreadCount > 9 ? '9+' : $unreadCount); ?></span>
                            <?php endif; ?>
                        </button>
                        <div x-show="open" x-cloak class="notif-panel absolute right-0 mt-2 w-96 rounded-xl border border-slate-200 bg-white p-4 shadow-xl dark:border-slate-800 dark:bg-slate-900">
                            ...
                            <div class="mt-3 max-h-80 space-y-2 overflow-y-auto">
                                <?php $__empty_1 = true; $__currentLoopData = $actor->notifications()->latest()->take(8)->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $notification): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <div class="rounded-lg p-3 <?php echo e($notification->read_at ? 'bg-slate-50 dark:bg-slate-950/40' : 'border-l-2 border-indigo-500 bg-indigo-50/50 dark:bg-indigo-950/20'); ?>">
                                        <div class="flex items-start justify-between gap-2">
                                            <div class="min-w-0">
                                                <div class="text-sm font-semibold text-slate-800 dark:text-slate-200"><?php echo e($notification->data['title'] ?? 'Notifikasi'); ?></div>
                                                <div class="mt-0.5 text-xs text-slate-500 dark:text-slate-400"><?php echo e($notification->data['message'] ?? '-'); ?></div>
                                                <?php if(!empty($notification->data['ticket_code'])): ?>
                                                    <span class="mt-1 inline-block rounded bg-slate-200 px-1.5 py-0.5 font-mono text-[10px] text-slate-600 dark:bg-slate-800 dark:text-slate-400"><?php echo e($notification->data['ticket_code']); ?></span>
                                                <?php endif; ?>
                                            </div>
                                        ...
                                        <?php if(!empty($notification->data['url'])): ?>
                                            <a href="<?php echo e($notification->data['url']); ?>" class="mt-2 inline-block text-xs font-semibold text-indigo-600 hover:underline dark:text-indigo-400">Buka tiket →</a>
                                        <?php endif; ?>
                                    </div>
                    ...
                    <div class="hidden items-center gap-2 border-l border-slate-200 pl-3 sm:flex dark:border-slate-800">
                        <div class="flex h-8 w-8 items-center justify-center rounded-full bg-slate-200 text-xs font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-300"><?php echo e(strtoupper(substr($actor->name, 0, 1))); ?></div>
                        <div>
                            <div class="text-sm font-semibold text-slate-800 dark:text-white"><?php echo e($actor->name); ?></div>
                            <div class="text-[11px] capitalize text-slate-400 dark:text-slate-500"><?php echo e($actor->role); ?></div>
                        </div>
                    </div>
        ...
        <div class="flex min-h-[calc(100vh-4rem)]">
            ...
            <aside class="hidden w-56 shrink-0 border-r border-slate-200 bg-white lg:block dark:border-slate-800 dark:bg-slate-900">
                <nav class="space-y-0.5 px-2 py-4">
                    ...
                    <?php endif; ?>
                    <a href="<?php echo e(route('home')); ?>" class="sidebar-link flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-600">
