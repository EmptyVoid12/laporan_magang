<div class="bg-white rounded shadow p-6">
    <div class="mb-6 overflow-hidden rounded-[28px] border border-sky-100 bg-gradient-to-r from-sky-50 via-white to-cyan-50 shadow-sm">
        <div class="flex flex-col gap-6 p-6 lg:flex-row lg:items-center lg:justify-between">
            ...
            <form
                method="GET"
                action="<?php echo e(route('technician.monthly-history.export')); ?>"
            ...
            <div class="border rounded-lg shadow-sm <?php echo e($task->status == 'Selesai' ? 'border-green-300 bg-green-50' : 'border-blue-300 bg-blue-50'); ?> flex flex-col h-full">
                <div class="p-4 border-b <?php echo e($task->status == 'Selesai' ? 'border-green-200' : 'border-blue-200'); ?>">
                    <div class="flex justify-between items-start mb-2">
                        <div>
                            <h3 class="font-bold text-lg text-gray-800"><?php echo e($task->perangkat->nama_perangkat); ?></h3>
                            <div class="text-xs font-mono text-blue-700"><?php echo e($task->kode_tiket); ?></div>
                        </div>
                        <span class="px-2 py-1 rounded text-xs font-semibold
                                <?php if($task->status == 'Selesai'): ?> bg-green-500 text-white
                                <?php elseif($task->status == 'Proses'): ?> bg-yellow-400 text-white
                                <?php elseif($task->status == 'Diverifikasi'): ?> bg-blue-500 text-white
                                <?php elseif($task->status == 'Menunggu'): ?> bg-orange-400 text-white
                                <?php elseif($task->status == 'Ditolak'): ?> bg-gray-500 text-white
                ...
                <div class="p-4 flex-1 flex flex-col justify-between">
                    ...
                    <?php elseif($task->isFinallyVerified()): ?>
                    ...
                    <div class="mt-4 flex gap-2">
                        <!--[if BLOCK]><![endif]--><?php if(!in_array($task->status, ['Selesai', 'Ditolak'])): ?>
                            ...
                            <button wire:click="updateTaskStatus(<?php echo e($task->id); ?>, 'Menunggu')" class="flex-1 bg-orange-500 hover:bg-orange-600 text-white text-sm font-bold py-2 px-3 rounded shadow">
                                Menunggu
                            </button>
                            <button wire:click="openCompletionModal(<?php echo e($task->id); ?>)" class="flex-1 bg-green-600 hover:bg-green-700 text-white text-sm font-bold py-2 px-3 rounded shadow">
                                √ Ajukan Selesai
                            </button>
                        <?php else: ?>
                            <div class="w-full text-center <?php echo e($task->status == 'Selesai' ? 'text-green-700 bg-green-100' : 'text-gray-700 bg-gray-100'); ?> font-bold py-2 rounded">
...
unset($__errorArgs, $__bag); ?><!--[if ENDBLOCK]><![endif]-->
                ...
                <div class="flex justify-end gap-3 border-t pt-4">
                    ...
                    <button type="submit" class="<?php echo e($modalMode === 'completion' ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'); ?> text-white font-bold py-2 px-4 rounded shadow transition">
                        <?php echo e($modalMode === 'completion' ? 'Kirim untuk Verifikasi' : 'Simpan Progress'); ?>
